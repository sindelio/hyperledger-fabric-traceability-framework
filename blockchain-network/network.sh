...
function printHelp() {
  ...
  echo "      - 'upgradecc'  - upgrade the chaincode in a running Fabric network"
...
function clearContainers() {
  ...
  CONTAINER_IDS=$(docker ps -a | awk '!/CONTAINER/ {print $1}')
...
function removeUnwantedImages() {
  ...
  DOCKER_IMAGE_IDS=$(docker image ls | grep dev-peer* | awk '{print $3}')
...
function networkUp() {
  ...
  docker exec cli scripts/start.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
# Upgrade the chaincode in a running Fabric network
function upgradeChaincode() {
  echo "Running upgrade-chaincode.sh"
  docker exec cli scripts/upgrade-chaincode.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Upgrade chaincode failed"
    exit 1
  fi
}
...
#
# use node as the default language for chaincode. Others: golang, java
LANGUAGE=node
# default image tag
# IMAGETAG="1.4.3"
IMAGETAG="latest"
...
if [ "$MODE" == "up" ]; then
  ...
  EXPMODE="Upgrading the chaincode"
else
  printHelp
  exit 1
fi
...
if [ "${MODE}" == "up" ]; then ## Create the network using docker compose
  ...
  upgradeNetwork
elif [ "${MODE}" == "upgradecc" ]; then ## Upgrade the chaincode in a running Fabric network
  upgradeChaincode
else
  printHelp
  exit 1
